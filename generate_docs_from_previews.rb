...
class ComponentDocumentationGenerator
  ...
  def parse_component_file(file_path)
    ...
    if content.match(/def\s+view_template\s*\(\s*&/) || content.match(/block_given\?/)
      ...
      elsif content.match(/block_given\?/)
        data[:block_usage] = :content
      end
    end
  ...
  def generate_example_params(component)
    ...
    component[:props].each do |prop|
      ...
      case prop[:name]
      when "field_name"
        example_props << "field_name: \"field_name\""
      when "label"
        example_props << "label: \"Label\""
      when "title"
        example_props << "title: \"Title\""
      when "text"
        example_props << "text: \"Text content\""
      when "icon_name", "icon"
        example_props << "#{prop[:name]}: \"star\""
      when "name"
        # Check if this is likely an icon name based on parent component
        if component[:name].include?("Icon")
          example_props << "name: \"star\""
        else
          example_props << "name: \"field_name\""
        end
      when "href", "path", "url"
        example_props << "#{prop[:name]}: \"/path\""
      when "description"
        example_props << "description: \"Description text\""
      when "content"
        example_props << "content: \"Content here\""
      when "value"
        example_props << "value: \"value\""
      when "id"
        example_props << "id: \"unique_id\""
      when "key"
        example_props << "key: \"key_name\""
      when "type"
        example_props << "type: :default"
      when "method"
        example_props << "method: :post"
      when "action"
        example_props << "action: \"/submit\""
      else
        # For unknown property names, use a generic format
        example_props << "#{prop[:name]}: \"...\""
      end
    end
