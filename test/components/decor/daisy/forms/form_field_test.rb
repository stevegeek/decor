...
class ::Decor::Daisy::Forms::FormFieldTest < ActiveSupport::TestCase
  # Concrete subclass — the FormField base is abstract (no view_template
  # of its own). This stand-in just renders the root element so we can
  # exercise class composition and state predicates.
  class TestField < ::Decor::Daisy::Forms::FormField
    def view_template(&)
      root_element do
        plain "field-body"
      end
  ...
  test "root element renders the FormField stimulus identifier" do
    html = render_component(TestField.new(name: "n"))
    assert_includes html, "decor--components--forms--form-field"
  end
