...
class Decor::Daisy::Modals::ModalOpenButtonTest < ActiveSupport::TestCase
  test "renders successfully with required modal_id" do
    component = Decor::Daisy::Modals::ModalOpenButton.new(modal_id: "test-modal")
    rendered = render_component(component)

    assert_includes rendered, "decor--daisy--modals--modal-open-button"
    assert_includes rendered, "btn"
  end

  test "inherits from Button component" do
    component = Decor::Daisy::Modals::ModalOpenButton.new(modal_id: "modal-1")

    assert component.is_a?(Decor::Components::Button)
  end

  test "renders with modal-opening data attributes" do
    component = Decor::Daisy::Modals::ModalOpenButton.new(modal_id: "my-modal")
    rendered = render_component(component)

    # Should have data attributes for opening modal
    assert_includes rendered, 'data-action="click->decor--daisy--modals--modal-open-button#handleButtonClick"'
  end

  test "renders with correct modal controller" do
    component = Decor::Daisy::Modals::ModalOpenButton.new(modal_id: "target-modal")
    rendered = render_component(component)
  ...
  test "supports button label" do
    component = Decor::Daisy::Modals::ModalOpenButton.new(modal_id: "text-modal", label: "Open Modal")
    rendered = render_component(component)
  ...
  test "supports button styling options" do
    component = Decor::Daisy::Modals::ModalOpenButton.new(
  ...
  test "handles modal_id as required attribute" do
    component = Decor::Daisy::Modals::ModalOpenButton.new(modal_id: "required-modal")

    assert_equal "required-modal", component.modal_id
  end

  test "renders as button element" do
    component = Decor::Daisy::Modals::ModalOpenButton.new(modal_id: "button-modal")
  ...
  test "renders with Stimulus data attributes for modal interaction" do
    component = Decor::Daisy::Modals::ModalOpenButton.new(modal_id: "stimulus-modal")
    fragment = render_fragment(component)
    ...
    assert_includes data_action, "decor--daisy--modals--modal-open-button"
  end
  ...
  test "handles various modal ID formats" do
    ...
    test_ids.each do |modal_id|
      component = Decor::Daisy::Modals::ModalOpenButton.new(modal_id: modal_id)
      rendered = render_component(component)
