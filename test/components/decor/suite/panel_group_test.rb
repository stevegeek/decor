...
class ::Decor::Suite::PanelGroupTest < ActiveSupport::TestCase
  ...
  test "omits header chrome entirely when title/description/cta all absent" do
    ...
    # No header border-b row when there's no title/desc/cta
  ...
  test "first panel row has no top divider, subsequent rows do" do
    ...
    # At least one section-divider border-t should be present (for the 2nd row)
  ...
  test "all generated tailwind utility classes are decor:-prefixed" do
    ...
    # extract every class= attribute and assert each Tailwind utility token is decor:-prefixed.
    # BEM-style component identifiers (e.g. "decor--suite--panel-group") are not Tailwind utilities.
