...
class ::Decor::Suite::Tables::DataTableFooterTest < ActiveSupport::TestCase
  ...
  test "applies the Confinus footer padding/gap layout" do
    html = render_component(::Decor::Suite::Tables::DataTableFooter.new)
  ...
  test "uses decor: prefix on every Tailwind class (no naked utilities)" do
    ...
    # Sample-check a handful of utilities we know are present
    assert_includes html, "decor:flex"
    # No unprefixed utilities — class strings should never contain a bare " flex" / " border-t" / " bg-suite-gray-25"
    refute_match(/class="[^"]*(?<![\w:])flex(?![\w-])[^"]*"/, html)
    refute_match(/class="[^"]*(?<![\w:])border-t(?![\w-])[^"]*"/, html)
    refute_match(/class="[^"]*(?<![\w:])bg-suite-gray-25(?![\w-])[^"]*"/, html)
  end

  test "does not leak daisyUI semantic colors" do
    html = render_component(::Decor::Suite::Tables::DataTableFooter.new(
      summary_lines: [FooterSummaryLine.new(title: "Total", value: "$5", separator: :final)],
      message: "x"
    ))
    refute_includes html, "decor:bg-base-50"
    refute_includes html, "decor:bg-base-300"
    refute_includes html, "decor:divide-base-300"
