...
class ::Decor::Suite::PropertyCardTest < ActiveSupport::TestCase
  ...
  test "rows layout omits the body column grid wrapper and uses row-layout properties" do
    ...
    # row layout puts label and value on a 2-col internal grid via Suite::Property
  ...
  test "does not use daisyUI semantic colors or non-suite border tokens" do
    html = render_component(::Decor::Suite::PropertyCard.new(title: "T", accent: :success)) do |card|
      card.with_property(label: "L", value: "V")
    end
    refute_includes html, "decor:border-l-primary-600"
    refute_includes html, "decor:border-l-success-600"
    ...
    refute_includes html, "decor:border-hairline"
