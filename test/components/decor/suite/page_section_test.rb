...
class ::Decor::Suite::PageSectionTest < ActiveSupport::TestCase
  ...
  test "body block is wrapped with suite-section-pad padding by default" do
    ...
    # both header and body should carry suite-section-pad — at least 2 occurrences
  ...
  test "padding :none drops the body padding utility" do
    ...
    # header still padded; only one suite-section-pad occurrence (the header)
  ...
  test "tag chips render via Suite::Tag (suite palette, not daisy)" do
    ...
    # Suite::Tag filled-success uses suite-success-50 bg
  ...
  test "background :primary swaps the surface fill to suite-primary-50" do
    ...
    # still carries hairline border + suite-card chrome
    assert_includes html, "decor:border-suite-hairline"
    assert_includes html, "decor:rounded-suite-card"
  end
  ...
  test "does not leak daisy semantic surface tokens (bg-base-100 / border-base-300)" do
    html = render_component(::Decor::Suite::PageSection.new(title: "T", description: "D", separator: true)) { "x".html_safe }
