...
module Navigo
  class Footer < PhlexComponent
    ...
    def convert_data_structures
      @link_groups = @link_groups.map do |group|
        case group
        when Hash
          LinkGroup.new(
            title: group[:title] || group["title"],
            links: (group[:links] || group["links"] || []).map { |l| convert_link(l) },
            visible: group.fetch(:visible, group.fetch("visible", true))
          )
        when LinkGroup
          group
        else
    ...
    def convert_link(link_data)
      case link_data
      when Hash
        Link.new(
          label: link_data[:label] || link_data["label"],
          href: link_data[:href] || link_data["href"],
          external: link_data.fetch(:external, link_data.fetch("external", false)),
          icon: link_data[:icon] || link_data["icon"]
        )
      when Link
        link_data
      else
