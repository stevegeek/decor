...
module Decor
  module Suite
    # Suite skin of Progress. Two display modes (driven by the abstract
    # base's `style` prop):
    #
    # - `:steps`   — clean-room step indicator: circle markers + small
    #               dot connectors. Horizontal on lg+, vertical-stacked on
    #               mobile. Connectors tint to suite-primary-400 once the
    #               step before them is completed.
    # - `:progress` — linear progress bar: muted suite-{color}-50 track +
    #               saturated suite-{color}-500 fill.
    # - `:both`     — bar on top, hairline divider, steps below.
      ...
      def render_step_li(idx, total)
        ...
        li(
          ...
          div(class: head_classes) do
            # Desktop leading connector (empty spacer on first step)
            ...
            # Desktop trailing connector (empty spacer on last step)
            ...
            # Mobile trailing vertical dots (below circle)
            unless is_last
