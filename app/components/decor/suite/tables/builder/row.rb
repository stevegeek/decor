...
module Decor
  module Suite
    module Tables
      module Builder
        # Pure-Ruby data carrier describing one row of the Suite DataTable
        # builder pipeline. Holds the built cells plus row-level settings
        # (selection, highlight, path, form_builder). `#component` returns the
        # kwargs hash a caller splats into `::Decor::Suite::Tables::DataTableRow.new`.
        #
        # Notes on the port:
        # - The original Confinus class mixed in `LiteralExtensions::StructHelpers`
        #   (merge/+/== shims from the pre-Literal `TypedStruct` era). Not carried
        #   over — the standard `Literal::Struct` API is enough for builder use.
