...
module Decor
  module Suite
    # Suite PageSection — sectioning block within a page. Carries a card-chromed
    # surface (white + suite-hairline border + rounded-suite-card corners),
    # a header strip (title + description + optional tag chips + cta cluster)
    # separated from the body by a suite-hairline rule, and an optional hero
    # block above the header.
    #
    # Visual chrome:
    # - Root: white surface, suite-hairline border, rounded-suite-card corners.
    # - Header: padded via `suite-section-pad`, title uses `suite-section-title`
    #   typography on gray-900, description uses `suite-description` on gray-500.
    # - Body: padded via `suite-section-pad`, vertical rhythm via
    #   `suite-section-gap` (mapped through @size for content area spacing).
    # - Separator: when `separator: true` AND header content present, the body
    #   sits below a `border-t border-suite-hairline` rule. When `separator:
    #   false`, the header and body share the same surface with no rule between.
    # - Background variants (:primary/:secondary/:neutral) tint the whole card
    #   with a translucent overlay over the white surface.
