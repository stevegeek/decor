...
module Decor
  module Suite
    # Suite Button — solid/outlined/ghost button with Stripe-style bevelled
    # shadow on filled variants.
    #
    # Visual chrome:
    # - Filled (default) uses a saturated primary/danger background or a
    #   muted warning pale-50 surface; layered inset highlight + ring +
    #   contact shadow give the Stripe-like elevated look.
    # - Outlined uses bg-white with a strong-hairline (or color-300 for
    #   danger) border, hover-bg-gray-50.
    # - Ghost (text-only) drops bg/border, hover gets a subtle gray-100
    #   surface and a darker text shade.
    # - Sizes follow the standard scale plus Suite-specific :wide
    #   (wider md) and :link (underlined inline text — no padding/bg).
    # - `loading:` overlays an inline spinner and hides the label via
    #   `text-transparent` so the button retains its width.
    class Button < ::Decor::Components::Button
      # Suite defaults: filled, primary, medium.
      default_style :filled
      default_color :primary
      default_size :md

      # Suite style scheme is the same conceptual set as the base; restated
      # here for documentation/discoverability.
      redefine_styles :filled, :outlined, :ghost

      # Suite drops the gem's accent/secondary/neutral/info palette and keeps
      # :base for the neutral white button.
      redefine_colors :base, :primary, :error, :warning

      # Add Suite-specific size knobs on top of the standard scale.
      # :wide — :md height + extra horizontal padding (form CTAs).
      # :link — no padding/bg, underlined inline text.
      ...
      # Filled ("contained") — saturated bg + white text + the layered
      # Stripe-style bevelled shadow on primary/danger. Warning uses the
      # muted pale-50 surface with dark text. :base falls back to the
      ...
      # Outlined — white bg + colored or hairline-strong border + dark text;
      # hover swaps to a muted pale background.
      ...
      # Ghost ("text") — no chrome until hover; hover paints a subtle
      # gray-100 surface and bumps the text one shade darker.
      def ghost_classes
        case @color
        when :primary
