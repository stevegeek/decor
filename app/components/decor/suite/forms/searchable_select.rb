...
module Decor
  module Suite
    module Forms
      # Suite skin of SearchableSelect — single-select typeahead with an
      # inline search input, a results dropdown, and a chip that replaces the
      # input once a value is picked.
      #
      # Visual identity:
      #   - Single suite-control-radius shell with hairline-strong border,
      #     white surface, 3px primary-100 focus halo
      #   - Dropdown rendered inline (absolute positioning relative to the
      #     shell) — sibling, so it inherits the shell's width and follows
      #     it in the layout. No floating-ui dependency.
      #   - Suite-token typography + suite-gray-25 hover row in the dropdown.
      #
      # The Stimulus controller (decor/suite/forms/searchable_select_controller)
      # owns open/close, XHR, debounce, keyboard nav, and click-outside dismiss.
      # The browser's popover API isn't used here because the dropdown needs
      # to sit in normal flow under the input shell for width inheritance and
      # because the input keeps keyboard focus across re-opens.
