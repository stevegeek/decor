...
module Decor
  module Suite
    module Forms
      # Suite MultiImageUpload — drag-to-reorder grid of image thumbnails
      # backed by SortableJS + CropperJS in the Stimulus controller. There is
      # no Daisy peer for this component (it shipped Suite-first), but the JS
      # controller is still registered under the Daisy path
      # (`decor--daisy--forms--multi-image-upload`) for symmetry with the
      # other Suite form controls. The Suite class name carries the implied
      # controller via the inherited `stimulus do` block; this class then
      # APPENDS the Daisy controller path so data-controller resolves both
      # identifiers and all `data-{daisy-id}-target` / `data-action` attrs
      # bind against the controller that actually owns the JS behaviour.
      #
      # Server-rendered chrome:
      #   - responsive thumbnail grid (sortable container target)
      #   - per-thumbnail Crop + Remove action buttons (revealed on hover)
      #   - "Primary" badge on the first thumbnail
      #   - "Add Images" button (Suite outlined button) + "n / max" counter
      #   - hidden native file input (multi-file picker)
      #   - hidden container the JS controller fills with image_order +
      #     remove_image_ids inputs as the user reorders / removes
      #   - hidden `new_images[]` file input the JS controller stocks via the
      #     DataTransfer API on every change so a single form submit ships
      #     the picked + cropped files
      #   - crop modal scaffolding (image + Cancel / Apply Crop buttons) —
      #     CropperJS attaches in the controller's `cropImage` action.
      #
      # All chrome uses Suite tokens (suite-hairline-strong borders,
      # rounded-suite-control, suite-primary-500 accents, suite-gray-25
      # surfaces, suite-danger-500 destructive accent, suite-field-label /
      # suite-field-help typography, duration-suite-fast motion).
