...
module Decor
  module Suite
    module Forms
      # Suite skin of the Radio form field.
      #
      # Renders a 16px circle with a 1.5px hairline border and a 8px filled
      # primary dot that fades / scales in when checked. Focus ring uses a 3px
      # primary-100 halo. The native <input> is overlaid on the label with
      # opacity 0.01 so point-based clicks (Selenium/Capybara) land on it
      # directly while keyboard focus and screen-reader semantics remain.
      #
      ...
      # (matches the Checkbox/Switch inline-chrome precedent so the Suite
      # form-layout primitives can evolve independently).
      class Radio < ::Decor::Components::Forms::Radio
        ...
        # Renders helper text and/or error text below the control. Mirrors the
        # historical radio's helper/error layout but uses suite tokens so we
        # don't depend on a Suite FormFieldLayout/HelperTextSection that
