...
module Decor
  module Suite
    # Suite PropertyList — vertical sibling to PropertyStrip. Renders one
    # or more sections of label/value rows inside a muted card surface
    # (white fill, suite-hairline edge, rounded-suite-card corners) with
    # an optional suite-section-title heading and a right-aligned CTA
    # slot. Section kickers use the suite-caption (uppercase) token, and
    # sections are separated by suite-hairline dividers. The `:grid`
    # layout wraps each section's contents in a CSS grid (2/3/4 columns).
    #
    # Section bodies typically contain `Decor::Suite::Property`
    # components — compose those instead of recreating row chrome here.
