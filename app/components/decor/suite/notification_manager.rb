...
module Decor
  module Suite
    # Suite NotificationManager — fixed-position stack that the JS controller
    # appends Notification toasts into.
    #
    # The container is positioned via Tailwind utilities (no hand-maintained CSS):
    # one of six edges, with `_center` variants horizontally centred via
    # `left-1/2 -translate-x-1/2`. Stack direction is `column` for top-anchored
    # positions (newest on top) and `column-reverse` for bottom-anchored
    # positions (newest on bottom).
    #
    # The wrapper itself is `pointer-events-none` so dead space doesn't block
    # the page; individual toasts re-enable pointer events on themselves.
    #
    # A `<template>` containing a Suite Notification is emitted alongside the
    # container so the shared Daisy controller can clone it into populated
    # toasts.
