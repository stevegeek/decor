...
module Decor
  module Suite
    # Suite Box — horizontal row container for settings/list items.
    # Visual identity: gray-25 surface, suite-hairline border,
    # rounded-suite-card chrome. Left-aligned title + description, with an
    # optional right-aligned actions cluster. Use `left` to fully replace the
    # left column, `right` to fill the actions cluster, or pass a block to
    # populate the actions cluster inline.
