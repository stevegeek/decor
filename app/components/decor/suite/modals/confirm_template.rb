...
module Decor
  module Suite
    module Modals
      # Singleton confirm-modal template rendered once into a host page (typ.
      # an overlays partial) inside a <template id="..."> element. A page-level
      # JS helper (e.g. spawnConfirmDialog) clones this on demand, fills the
      # named targets with caller-supplied content, picks the variant via the
      # Stimulus classes API below, and calls showModal() on the resulting
      # <dialog>.
      #
      # The Stimulus `classes` DSL block is the single source of truth for
      # every variant -> colour mapping. The cloning controller reads e.g.
      # `this.accentInfoClasses` rather than carrying its own VARIANT_*_CLASS
      # maps in JS.
      #
      # Targets exposed to the controller (every slot the controller fills at
      # runtime):
      #   accent         - left accent bar (gets one accent_<variant> class)
      #   icon_info / icon_success / icon_warning / icon_danger /
      #     icon_destructive - pre-rendered variant icons, all hidden; the
      #                        controller un-hides exactly one
      #   title          - h3; controller sets textContent and applies
      #                    destructive_title classes for :destructive
      #   message        - p body; controller sets textContent
      #   cancel_button  - cancel/negative button (rendered twice: the close
      #                    X in the header and the footer Cancel button)
      #   confirm_button - confirm/positive button; controller swaps to
      #                    danger chrome for :destructive
      #   header         - header div; controller applies destructive_header
      #                    classes for :destructive
      class ConfirmTemplate < ::Decor::PhlexComponent
        include ConfirmShared

        stimulus do
          # Variant -> accent bar colour class. JS reads this.accentInfoClasses
          # via the Stimulus classes API; values come from
          # ConfirmShared::VARIANT_ACCENT_CLASSES (single source of truth).
          classes accent_info: ConfirmShared::VARIANT_ACCENT_CLASSES[:info],
            ...
            # Variant -> icon colour class (ConfirmShared::VARIANT_ICON_COLOR_CLASSES).
            ...
            # Destructive header/title treatment - delegated to Suite::Modals::Modal
            # constants so the JS-spawned path and the server-rendered path
            # stay byte-identical.
