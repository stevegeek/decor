...
module Decor
  module Suite
    module Modals
      # Suite skin of ModalCloseButton. Renders a button that, when clicked,
      # dispatches a window-scoped close event the Suite Modal listens for.
      # The optional `close_reason` is carried on the event detail so listeners
      # can branch on intent (e.g. "save" vs "cancel" vs "delete").
      #
      # Standalone-capable: the Suite Modal companion need not be present;
      # any listener bound to the dispatched event can react.
      class ModalCloseButton < ::Decor::Components::Modals::ModalCloseButton
          ...
          root_element do
            span(class: "decor:inline-flex decor:items-center decor:gap-1.5") do
              if @icon
                ...
                # Default to an X glyph when no label / no icon was specified —
                # mirrors the historical Suite "icon-only close" affordance.
        ...
        # Color + style combine to produce a single visual treatment matched
        # to the Suite identity. Defaults to a neutral chrome (white surface,
        # hairline border, gray text) which is the historical Suite default
        # for unlabeled close affordances.
