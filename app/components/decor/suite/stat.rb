...
module Decor
  module Suite
    # Suite Stat — single tile presenting a labeled metric value and an
    # optional delta/figure indicator. Visually coherent with the Suite
    # property family: white surface, hairline border, rounded-suite-card
    # padding box; not the daisyUI horizontal stat slab.
    #
    # Slot mapping (inherits from ::Decor::Components::Stat):
    # - `title:` → small uppercase-ish kicker label (`suite-caption`).
    # - `value:` (or block) → headline value (`suite-section-title`).
    # - `description:` → delta/figure line below the value
    #   (`suite-description`). Color is driven by `delta:`.
    # - `figure { ... }` → optional trailing block (e.g. sparkline) rendered
    #   in the figure slot at the top-right. Falls back to `icon:` if no
    #   figure block was given.
    # - `actions { ... }` → optional footer action row.
    #
    # `delta:` is the Suite addition on top of the abstract base. `:up`
    # paints the description in `suite-primary-700`, `:down` in
    # `suite-danger-700`, `:none` (default) leaves it gray.
