...
module Decor
  module Suite
    # Suite SwitchingBox — settings-row container with a title/description on
    # the left and a form-bound switch on the right that submits on change.
    # Visual identity: white surface, suite-hairline border, rounded-suite-card
    # chrome, hairline-strong hover. Set `stack: true` on adjacent rows to
    # collapse internal borders into a single grouped list (first row keeps the
    # top radius, last row keeps the bottom radius, middle rows are flush).
