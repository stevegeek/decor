...
module Decor
  module Daisy
    module Nav
      # CompactFooter A condensed centered footer variant. Renders social
      # icons, footer links, an optional logo, and a copyright line.
      # Subclasses Decor::Components::Nav::CompactFooter for the prop API
      # and inherits Footer's inner-class constants via the abstract base.
      class CompactFooter < ::Decor::Components::Nav::CompactFooter
        def view_template(&)
          vanish(&)
          root_element do
            div(class: "decor:container decor:mx-auto decor:px-4 decor:py-6") do
        ...
        def root_element_classes
          classes = ["decor:d-footer", "decor:d-footer-center"]
          classes << ((@theme == :dark) ? "decor:bg-neutral decor:text-neutral-content" : "decor:bg-base-300")
          classes.join(" ")
        end
        ...
        def render_social_section
          ...
          div(class: "decor:flex decor:justify-center decor:space-x-4 decor:mb-4") do
        ...
        def render_links_section
          ...
          nav(class: "decor:flex decor:flex-wrap decor:justify-center decor:gap-x-6 decor:gap-y-2 decor:mb-4", aria_label: "Footer") do
            if @custom_links
              render @custom_links
            else
        ...
        def render_logo_section
          ...
          div(class: "decor:mb-4") do
            if @logo_content
              ...
              div(class: "decor:mx-auto decor:h-16 decor:w-auto decor:flex decor:items-center decor:justify-center decor:rounded") do
                span(class: "decor:font-bold decor:text-lg") { @company_name }
              end
            end
          end
        end
        ...
        def render_copyright_section
          div(class: "decor:text-center") do
            if @copyright_content
              render @copyright_content
            else
              p(class: "decor:text-xs decor:opacity-70") do
        ...
        def render_social_link(social_link)
          a(
            ...
            class: "decor:d-btn decor:d-btn-ghost decor:d-btn-circle decor:d-btn-sm",
        ...
        def render_footer_link(link)
          a(
            href: link.href,
            class: "decor:text-sm decor:d-link decor:d-link-hover",
        ...
        def render_social_icon(platform)
          case platform
          when :facebook
            svg(class: "decor:w-4 decor:h-4 decor:fill-current", viewBox: "0 0 24 24") do |s|
