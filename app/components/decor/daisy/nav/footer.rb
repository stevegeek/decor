...
module Decor
  module Daisy
    module Nav
      # Footer A multi-column site footer with optional newsletter signup,
      # link groups, social links, and a bottom bar. Themed light or dark.
      class Footer < ::Decor::Components::Nav::Footer
        def view_template(&)
          vanish(&)
          root_element do
            div(class: "decor:container decor:mx-auto decor:px-4 decor:py-8 decor:lg:py-12") do
              div(class: "decor:grid decor:grid-cols-1 decor:lg:grid-cols-3 decor:gap-8") do
        ...
        def root_element_classes
          classes = ["decor:d-footer"]
          classes << ((@theme == :dark) ? "decor:bg-neutral decor:text-neutral-content" : "decor:bg-base-200")
          classes.join(" ")
        end

        def render_content_section
          div(class: "decor:lg:col-span-1") do
            if @logo_content
              render @logo_content
            end
        ...
        def render_newsletter_section
          div(class: "decor:mt-6") do
            h3(class: "decor:d-footer-title") { "Subscribe to our newsletter" }
            p(class: "decor:text-sm decor:opacity-70 decor:mb-4") { "Stay in the loop with the latest features, updates, and efficiency-boosting tips." }

            render ::Decor::Daisy::Forms::Form.new(model: @leads_model, local: true, url: @leads_submit_path, html: {class: "decor:flex decor:flex-col decor:sm:flex-row decor:gap-2"}) do |form|
              raw(form.builder.hidden_field(:from_entry_point, value: "corp_site_footer").html_safe)
              raw(form.builder.email_field(:email, label_position: :inside, required: true, placeholder: "Enter your email", class: "decor:d-input decor:d-input-bordered decor:flex-1").html_safe)
              raw(form.builder.submit("Subscribe", class: "decor:d-btn decor:d-btn-primary").html_safe)
            end
          end
        end

        def render_links_section
          div(class: "decor:lg:col-span-2") do
            if @custom_links
              render @custom_links
            elsif @link_groups.any?
              div(class: "decor:grid decor:grid-cols-2 decor:md:grid-cols-#{[4, @link_groups.size].min} decor:gap-6") do
        ...
        def render_link_group(group)
          div do
            h3(class: "decor:d-footer-title") { group.title }
            ul(class: "decor:space-y-2") do
              group.links.each do |link|
                li do
                  a(
                    href: link.href,
                    class: "decor:d-link decor:d-link-hover",
        ...
        def render_bottom_section
          div(class: "decor:d-footer decor:d-footer-center decor:mt-8 decor:pt-6 decor:border-t decor:border-base-300") do
            if @show_social && @social_links.any?
              div(class: "decor:flex decor:flex-wrap decor:justify-center decor:gap-4 decor:mb-4") do
            ...
            div do
              if @copyright_content
                ...
                p(class: "decor:text-sm decor:opacity-70") do
        ...
        def render_social_link(social_link)
          a(
            ...
            class: "decor:d-btn decor:d-btn-ghost decor:d-btn-circle",
        ...
        def render_social_icon(platform)
          case platform
          when :facebook
            svg(class: "decor:w-5 decor:h-5 decor:fill-current", viewBox: "0 0 24 24") do |s|
