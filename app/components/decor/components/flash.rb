# frozen_string_literal: true

module Decor
  module Components
    # Abstract base for Flash. Owns the prop API + defaults + Stimulus
    # contract + skin-agnostic helpers (variant detection from Rails flash,
    # default titles, icon name mapping). Concrete skins (Daisy, Suite)
    # inherit and provide `view_template` plus class-builder overrides.
    class Flash < ::Decor::PhlexComponent
      stimulus do
        actions ["decor-flash:open@window", :handle_show_event], ["decor-flash:close@window", :handle_close_event]
        values show_initial: -> { show_initial? }
      end

      prop :title, _Nilable(String)
      prop :text, _Nilable(String)
      prop :preserve_flash, _Boolean, default: false
      prop :collapse_if_empty, _Boolean, default: true

      prop :flash_data, _Nilable(ActionDispatch::Flash::FlashHash)
      prop :controller_path, _Nilable(String)
      prop :action_name, _Nilable(String)

      # Flash uses semantic colors for alert types
      default_color :info

      private

      # Similar to https://guides.rubyonrails.org/i18n.html#lazy-lookup we
      # provide a way to define a default title for the flash based on the
      # controller and action name.
      def title_with_defaults
        return @title if @title.present?
        if @controller_path && @action_name
          string_key = "#{@controller_path}.#{@action_name}.flash.title.#{resolved_color}"
          return I18n.t(string_key) if I18n.exists?(string_key)
        end

        case resolved_color
        when :success
          "Success!"
        when :error
          "An error exists."
        when :warning
          "Attention needed"
        else
          "Notice"
        end
      end

      def icon
        case resolved_color
        when :success
          "circle-check"
        when :error
          "circle-x"
        when :warning
          "exclamation-mark"
        else
          "info-circle"
        end
      end

      def show_initial?
        text_with_default.present?
      end

      def text_with_default
        @text_with_default ||= if @text.present?
          @text
        else
          flash = resolved_flash
          set_variant(flash)
          text_sentences(flash[:success], flash_notice(flash), flash_alert(flash), flash[:errors]).tap do
            flash.clear unless @preserve_flash
          end
        end
      end

      def text_sentences(*sections)
        sections.flatten.compact.map(&:to_s).join(". ")
      end

      def set_variant(flash)
        @forced_color =
          if flash[:errors].present?
            :error
          elsif flash[:success].present?
            :success
          elsif flash_notice(flash).present?
            :neutral
          elsif flash_alert(flash).present?
            :warning
          else
            :info
          end
      end

      def resolved_color
        @forced_color || @color
      end

      def resolved_flash
        @flash_data || {}
      end

      def flash_notice(flash_obj)
        flash_obj.respond_to?(:notice) ? flash_obj.notice : flash_obj[:notice]
      end

      def flash_alert(flash_obj)
        flash_obj.respond_to?(:alert) ? flash_obj.alert : flash_obj[:alert]
      end
    end
  end
end
