...
module Decor
  module Components
    module Modals
      # Abstract base for Modals::Form. Owns the prop API for a Modal that
      # pre-wires a <form> body + a Cancel/Submit footer.
      #
      # The submit button lives in the footer (outside the form element) and
      # is associated via the HTML5 `form="..."` attribute, so the inner
      # <form> element in the caller's body must carry an id matching
      # `form_id` (auto-derived from the component's Vident id when not
      # explicitly passed).
      #
