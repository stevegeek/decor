...
module Decor
  module Components
    # Abstract base for SettingsList. Owns the prop API + nested data
    # carriers (Group, Row, ScopeInfo) + the groups/rows validation +
    # helpers used by skins. Concrete skins (Daisy, Suite) inherit and
    # provide `view_template` plus their visual chrome.
    #
    # Editable rows: when a row has `edit_path:`, skins render an edit
    # affordance (anchor by default). The host application supplies any
    # heavier interaction (modal triggers, intercept handlers) by wrapping
    # the link or providing a custom `value` block — the gem does not
    # depend on a modal subsystem.
